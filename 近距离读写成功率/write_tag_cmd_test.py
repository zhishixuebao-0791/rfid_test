...
#FM0_200KHz_6_25, FM0_40KHz_25,
#602 盘存速度一般 40KHz 写异常 换
#606盘存慢写异常           换
each_bank_write_whell = 10                     #每个区域要写多少轮
...
def make_file_return_path(path_txt:str):
    ...
    return path_txt+('\\write_success_rate_' + time.strftime('%Y%m%d%H%M') + '.txt')
#指令
set_freq = "A006017801073B9E"
set_addr ="A004007300E9"#设置读写器地址为0x00
realtime_inventory = "A004018900D2"
...
#每个区域每次写2byte的数据,读写其地址为0x00             
mem_bank_table = {
                    'WRITE_BANK_00':'A0120082000000000000000000000200000001C9',#密码区
                    'WRITE_BANK_01':'A0120082000000000100000002000212345678B3',#EPC区#EPC区起始地址为2
                    'WRITE_BANK_11':'A012008200000000030000000000022025090970' #USER区
                }
...
class SerThread():
    ...
    def write_tad(self):
        self.my_serial.write(send_write_tag(self.mem_bank))
...
if __name__ == '__main__':
    ...
    try:
        ...
        read_rate_file = open(make_file_return_path(path_txt), 'w')#打开文件
        ...
        for key_link, value_link in rf_link_table.items():
            ...
            for key_bank, value_bank in mem_bank_table.items():
                ...
                for num in range(each_bank_write_whell):
                    print('round : ', (num+1))
                    ser.mem_bank = value_bank
                    print("send_cmd:",ser.mem_bank)
                    ser.my_serial.reset_input_buffer()#清理接收缓冲区
                    ser.write_tad()#发写区域指令
                    if(not ser.start()):#子线程解析计数#解析
                        exit(1)
                    ser.waiting()#阻塞主线程等待子线程结束
                success_rate = (ser.read_success_num / each_bank_write_whell)*100#成功率
                print('read rate = {}%'.format(success_rate))
                read_rate_file.writelines("[" + key_bank + "] , "+"[" + key_link + "] , " +"[write rate = " + str(success_rate) + "%] \n")
