...
def Draw_Histogram(folder_path):
    # 遍历文件夹中的所有文件
    for filename in os.listdir(folder_path):#os.listdir() 方法用于返回指定目录下的所有文件和子目录的名称列表
        if filename.startswith("remote_read_counter") and filename.endswith(".txt"):
            ...
            plt.tight_layout()
            # 保存图表
            output_filename = f"{os.path.splitext(filename)[0]}_chart.png"#os.path.splitext分割文件名和扩展名例如os.path.splitext("document.pdf")>>('document', '.pdf')
            output_path = os.path.join(folder_path, output_filename)#路径拼接如os.path.join(path,abc)>>path\abc
