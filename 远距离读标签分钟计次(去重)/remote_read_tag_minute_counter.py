...
class SerThread():
    def __init__(self, portx):
        ...
        self.ucm60x_tag_epc_list = []
        self.ucm60x_tag_epc_num = 0
        self.epc_same_flag = 1
    ...
    def ucm60X_decode_rfid_packet(self):
        if (int(self.tag_package.len,16) > 12):
            if (int(self.tag_package.cmd,16) == 0x89):
                ...
                if(self.ucm60x_tag_epc_num > 0):
                    #去重
                    for i in range(self.ucm60x_tag_epc_num):
                        self.epc_same_flag = 1
                        if (epc_len == len(self.ucm60x_tag_epc_list[i])):
                            for j in range (len(self.ucm60x_tag_epc_list[i])):
                                if(self.tag_package.data[j+3] != self.ucm60x_tag_epc_list[i][j]):
                                    self.epc_same_flag = 0
                        else:
                            self.epc_same_flag = 0
                        if(self.epc_same_flag == 1):
                            #print("repetition_epc!"+("-"*50)+"repetition_epc!")
                            return
                    epc_list = []
                    epc_list.clear()#清空epc列表
                    print('\033[32m'+"insert new tag:",end=" ")
                    for i in range(epc_len):
                        epc_list.append(self.tag_package.data[i+3])
                        if(i == (epc_len-1)):
                            print(epc_list[i]+'\033[0m')
                        else:
                            print(epc_list[i],end=" ")
                    self.ucm60x_tag_epc_list.append(epc_list)
                    self.ucm60x_tag_epc_num += 1
                    #print("ucm60x_tag_epc_num = ",self.ucm60x_tag_epc_num,'\033[0m')
                    del epc_list
                else:
                    epc_list = []
                    epc_list.clear()#清空epc列表
    ...
    try:
        ...
        for key, value in rf_link_table.items():
            ...
            for num in range(run_ring):
                ...
                ser.ucm60x_tag_epc_list.clear()#清空列表
