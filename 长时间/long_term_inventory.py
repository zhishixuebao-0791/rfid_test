...
each_ring_tim_interval = 2                     #每轮之间的时间间隔单位s
#指令
...
def make_file_return_path(path_txt,fname):
    ...
    return path_txt+('\\long_term_inventory_' + fname + '.txt')
...
if __name__ == '__main__':
    ...
    try:
        ...
        for key, value in rf_link_table.items():
            ser.my_serial.reset_input_buffer()#清除输入缓冲区数据
            ...
            for num in range(run_ring):
                print("-"*23+"Start!"+"-"*23)
                read_rate_file.writelines("-"*25+"NEW LINE"+"-"*25+"\n")
                print("round: ",(num+1))
                read_rate_file.write("round: "+str(num+1)+"\n")
                ...
                list_len = len(ser.ucm60x_tag_epc_list_str)
                if list_len != 0:
                    for i in range(list_len):
                        read_rate_file.write(ser.ucm60x_tag_epc_list_str[i] + '\n')
                read_rate_file.write("ucm60x_tag_epc_num = "+str(ser.ucm60x_tag_epc_num)+"\n")        
                ser.ucm60x_tag_epc_list_str.clear()#清空列表
                ser.ucm60x_tag_epc_num = 0
                read_rate_file.writelines("-"*25+"END LINE"+"-"*25+"\n")
                read_rate_file.write("sleep: "+str(each_ring_tim_interval)+"s\n")
